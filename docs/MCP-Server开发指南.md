...
| 项 | 状态 |
|----|------|
| 工具规格（17 个） | ✅ 已定稿，见 `ES接口清单.xlsx` 与本指南 |
| SAP 底层 API 探测 | ✅ 24/33 端点 OK（见连通性手册） |
| 环境配置 `.env.example` | ✅ |
| 依赖 `node_modules` | ✅ 已安装（`@modelcontextprotocol/sdk` 等） |
| **源码** `mcp-server.js` 等 | ❌ **缺失，待实现** |
| stdio / HTTP 端到端验证 | ⏳ 待源码 |

历史日志 `_mcp_stderr.txt` 显示曾以 **sap-s4-mcp v0.1** 成功启动 stdio 模式，说明设计曾部分实现；当前磁盘上无源码文件。
...
│   ├── scenarios.js       # SAP_COM_* 场景 → URL 模板
│   └── tools/             # 各 MCP 工具 handler
├── .env
└── README.md
```
...
- [ ] 恢复/创建 `package.json`、`mcp-server.js`、`mcp-sap-core.js`
- [ ] 凭证解析（与探测脚本一致）
- [ ] stdio 传输 + `health_check`
- [ ] 7 个已验证工具：`get_sales_order_status`、`get_product`、`get_business_partner`、`get_purchase_order`、`get_material_stock`、`get_supplier_invoice`、`get_cost_center`

### Phase 2 — 增强

- [ ] `trace_sales_order`、`query_sap_scenario`
- [ ] HTTP 传输 + `authenticate`
- [ ] `list_sap_scenarios`、`get_entity_schema`
- [ ] 响应缓存（`SAP_CACHE_TTL_MS`）

### Phase 3 — 扩展场景

- [ ] 采购申请、计划协议、销售合同、BOM、预留单（随 SAP Arrangement 开通）
- [ ] 主数据描述字段（SAP_COM_0087 开通后）

### Phase 4 — 运维

- [ ] 单元测试（凭证解析、URL 构建）
- [ ] 与 `probe-sap-connectivity.js` 共享端点表
- [ ] Git 入库，`.env` / `user.txt` 加入 `.gitignore`
