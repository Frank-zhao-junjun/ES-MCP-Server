...
- [ ] T1: 设计 `trace-config.json` schema 并把 `TRACE_STEPS` 从 `sales-order-trace.js` 迁移进去
...
- [ ] T2: 在 `sales-order-trace.js` 中实现「模板替换 + JSON 配置驱动」的 `buildUrl`
...
- [ ] T3: 统一错误处理层级，消除 `mcp-server.js` 中的重复 `normalizeError` / `makeError` 混用
...
- [ ] T4: 为 `discoveryCache` 和 `_scenariosCache` 添加 mtime/TTL 失效
...
- [ ] T5: 启动时校验环境变量和文件路径，提取 `SAP_HOST` 到环境变量
...
- [ ] T6: 为 `query_sap_scenario` 添加 `skip` 参数，并在返回中提示「是否还有更多」
...
- [ ] T7: 新增 `get_entity_schema` Tool，解析 SAP `$metadata`
...
- [ ] T9: 将 `wrapTool` 的 `requestId` 注入到 SAP 子查询的 `X-Request-ID` header
...
- Date: 2026-06-11
- Completed task IDs: 阶段 1 已全部完成（输入校验、统一编码、health_check、结构化日志、graceful shutdown）
- Checks run: `node tests/run-tests.js` ✅
- Remaining risks: T3 改动面广；T7 解析 XML 可能引入新依赖
